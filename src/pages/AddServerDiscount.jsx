...
const AddServerDiscount = () => {
  ...
  const fetchDiscounts = async () => {
    try {
      const response = await axios.get("/api/server/get-server-discount");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post("/api/server/add-server-discount", {
        server: Id,
  ...
  const handleDelete = async (serverId) => {
    try {
      await axios.delete(`/api/server/delete-server-discount?server=${serverId}`);
      setDiscounts(
