    ...
    const checkMFAStatus = async () => {
      try {
        ...
        if (data.success && data.is2FAEnabled && data.is2FASetupComplete) {
          ...
          setTimeout(() => {
            login();
            navigate("/unsend-trx"); // Redirect to the unsend transaction page
          }, 2000);
        } else {
  ...
  const handleVerifyMFA = async (e) => {
    ...
    try {
      ...
      if (data.message === "2FA verified successfully. Access granted.") {
        login();
        navigate("/unsend-trx"); // Redirect to the unsend transaction page after successful MFA
        toast.success(
        ...
        <CardContent className="p-6">
          {!isMFAEnabled ? (
            <div className="text-center text-[#9d9d9d]">
              <p>MFA is disabled. Redirecting you to the unsend transaction page...</p>
            </div>
          ) : (
            <>
...
export default AppLayout()(MfapageforUnsendTrx);
