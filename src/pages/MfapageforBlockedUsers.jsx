    ...
    const checkMFAStatus = async () => {
      try {
        ...
        if (data.success && data.is2FAEnabled && data.is2FASetupComplete) {
          ...
          login();
          setTimeout(() => {
            navigate("/blocked-users");// Redirect to desired page
          }, 2000);
        } else {
  ...
  const handleVerifyMFA = async (e) => {
    ...
    try {
      ...
      if (data.message === "2FA verified successfully. Access granted.") {
        login();
        navigate("/blocked-users");// Navigate to the main page
        toast.success(
        ...
        <CardContent className="p-6">
          {!isMFAEnabled ? (
            <div className="text-center text-[#9d9d9d]">
              <p>MFA is disabled. Redirecting you to the Blocked User page...</p>
            </div>
          ) : (
            <>
...
export default AppLayout()(MfapageforBlockedUsers);
