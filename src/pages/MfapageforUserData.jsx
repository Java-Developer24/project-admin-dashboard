    ...
    const checkMFAStatus = async () => {
      try {
        ...
        if (data.success && data.is2FAEnabled && data.is2FASetupComplete) {
          ...
          login();
          navigate("/users-data") // Redirect to desired page
        } else {
  ...
  const handleVerifyMFA = async (e) => {
    ...
    try {
      ...
      if (data.message === "2FA verified successfully. Access granted.") {
        login();
        navigate("/users-data") // Navigate to the main page
        toast.success(
...
export default AppLayout()(MfapageforUserData);
