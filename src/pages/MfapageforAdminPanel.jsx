    ...
    const checkMFAStatus = async () => {
      try {
        ...
        if (data.success && data.is2FAEnabled && data.is2FASetupComplete) {
          ...
          login();
          setTimeout(() => {
            navigate("/admin-panel"); // Redirect to desired page
          }, 2000);
        } else {
    ...
    try {
      ...
      if (data.message === "2FA verified successfully. Access granted.") {
        ...
        login();
          ...
          navigate("/admin-panel");// Navigate to the main page
        ...
        <CardContent className="p-6">
          {!isMFAEnabled ? (
            <div className="text-center text-[#9d9d9d]">
              <p>MFA is disabled. Redirecting you to the Admin Panel ...</p>
            </div>
          ) : (
            <>
...
export default AppLayout()(MfapageforAdminPanel);
