  ...
  const handleVerifyMFA = async (e) => {
    ...
    try {
      ...
      if (data.message === "2FA verified successfully. Access granted.") {
        ...
        toast.error("Invalid MFA code.", { duration: 3000, position: "top-center" })
      }
    } catch (error) {
      console.error("Error verifying MFA:", error)
      toast.error("Failed to verify MFA code.", { duration: 3000, position: "top-center" })
    }
  }
      ...
      <Card className="bg-transparent w-full max-w-md rounded-lg mb-[60px] border-none dark">
        <CardHeader>
          <CardTitle className="text-center text-[28px] font-medium">
            {isMFAEnabled ? (isMFASetupComplete ? "Enter MFA Code" : "Setup MFA Authentication") : "MFA Disabled"}
          </CardTitle>
        </CardHeader>
          ...
          {!isMFAEnabled ? (
            ...
            <>
              ...
              {!isMFASetupComplete && qrCodeUrl && (
                <div className="mb-6">
                  <p className="text-[#9d9d9d] mb-4 text-center">Scan this QR code with your authenticator app</p>
                  <div className="flex justify-center mb-4">
                    <img src={qrCodeUrl || "/placeholder.svg"} alt="MFA QR Code" className="w-48 h-48" />
                  </div>
                </div>
              )}
              ...
              <form onSubmit={handleVerifyMFA}>
                <div className="space-y-4">
                  <div>
                    <Label htmlFor="mfaCode" className="block text-sm text-[#9d9d9d] font-normal py-1">
                      {isMFASetupComplete ? "Enter Code from Authenticator" : "Enter Code to Complete Setup"}
                    </Label>
                    <Input
                      id="mfaCode"
                      type="text"
                  ...
                  <Button type="submit" variant="login" className="w-full text-sm font-normal mt-4">
